...
SEXP FiltroKalmanCKLS(SEXP y, SEXP rti, SEXP delta, SEXP alpha, SEXP beta, SEXP gamma, SEXP A, SEXP Phi,
                      SEXP R, SEXP Q, SEXP xp, SEXP Pp, SEXP xf, SEXP Pf, SEXP K, SEXP innov, SEXP sig){
  int i, n;
  double siginv, like, *py, *prti, *pxp, *pPp, *pxf, *pPf, *pK, *pinnov, *psig;
  double rdelta, ralpha, rbeta, rgamma, rA, rPhi, rR, rQ;
  ...
  for (i = 1; i < n; i++) {
    ...
    pPp[i] = rPhi * pPf[i - 1] * rPhi + rQ * pow(prti[i],(2 * rgamma)) / rdelta;
